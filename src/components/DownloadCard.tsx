...
export const DownloadCard = ({ onDownloadClick }: DownloadCardProps) => {
  return (
    <div className="max-w-[350px] w-full p-5 rounded-[20px] mt-5" style={{
      ...
      <span className="text-sm font-semibold" style={{ color: '#488aec' }}>Bientôt disponible !</span>
      ...
      <p className="text-[13px] font-semibold mb-3" style={{ color: 'rgb(151 153 167)' }}>
        Découvrez bientôt ma nouvelle application pour créer du contenu multimédia par IA.
      </p>
      <div className="flex items-center justify-center gap-2.5 mt-2.5">
        <button
          disabled
          className="flex items-center justify-between rounded-[10px] px-3 py-1.5 cursor-not-allowed opacity-60"
          style={{
            backgroundColor: '#488aec',
            boxShadow: '0 4px 6px -1px #488aec31, 0 2px 4px -1px #488aec17'
          }}
        >
          <span className="text-white font-bold">Bientôt</span>
        </button>
      </div>
    </div>
  );
